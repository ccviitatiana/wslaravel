...
<div class="md:grid flex md:grid-cols-2 flex-col px-[5%] pb-20 pt-10 gap-20">
    <div class=''>
        ...
        <div class='grid grid-cols-1 gap-4 pb-10 mb-4 justify-items-start items-center'>
            ...
            <a href="{{ route('post', $post->slug) }}" class="bg-gray-200 rounded-lg px-6 pt-6 pb-3 w-full">
                <p class="text-xs flex items-center gap-2">
                    <span class="uppercase text-gray-200 bg-gray-900 rounded-full px-2 py-1">Tutorial</span>
                    <span>{{ $post->created_at->format('d/m/Y') }}</span>
                </p>
                <h2 class="text-lg text-gray-900 mt-2 px-6 pt-4">{{ $post->title }}</h2>
                <img src="{{ url('images/' . $post->image_path) }}" alt="">
                <div class="bg-gray-200 rounded-lg flex items-center grow-2 justify-between flex-grow gap-8 bg-gray-200 px-6 py-4 w-full">
                    <img id="reaction" onclick="reaction" src="{{ URL( '/reaction-off.png' ) }}" class="size-6" alt="">
